...
public interface Selectable
{
	 ...
	 */
	default void selectRange(int from, int to, @Nullable Action action)
	{
		final IntFunction<Action> function = action != null ? index -> action : null;
		selectRange(from, to, function);
	}

	/**
	 * Select the list items in the range <code>[from,&nbsp;to[</code> according
	 * to the given function. The whole function as well as individual function
	 * values can be {@code null} and work as a no-op. The indices work like:
	 * <pre><code>for (int i = from; i &lt; to; i++)</code></pre>
	 *
	 * @param from      the beginning of the index range (including)
	 * @param to        the end of the index range (excluding)
	 * @param predicate the selection action to apply, {@code null} is a no-op
	 * @since 2.1
	 */
	default void selectRange(int from, int to, @Nullable IntPredicate predicate)
	{
		final IntFunction<Action> function = predicate != null ?
			(index -> predicate.test(index) ? SELECTION_SET : SELECTION_UNSET) : null;
		selectRange(from, to, function);
	}
	 ...
	 * @param from     the beginning of the index range (including)
	 * @param to       the end of the index range (excluding)
	 * @param function the selection action to apply, {@code null} is a no-op
	 * @since 2.1
	 */
	void selectRange(int from, int to, @Nullable IntFunction<@Nullable Action> function);
	...
	/**
	 * Set the selection status of all list items according to the given function.
	 *
	 * @param function the given index to action function
	 * @since 2.1
	 */
	default void selectAll(IntFunction<Action> function)
	{
		selectRange(0, size(), function);
	}
